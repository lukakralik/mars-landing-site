...
class Search():
    ...
    def run_sect_stats(self):
        ...
        while True:
            rect_img = GRAY_IMG[ul_y: lr_y, ul_x: lr_x]
            self.rect_coords[rect_num] = [ul_x, ul_y, lr_x, lr_y]
            if np.mean(rect_img) <= MAX_ELEV:
    ...
    def sort_stats(self):
    	...
    	self.ptp_filtered = [x[0] for x in ptp_sorted[:N_CANDIDATES]]
    	std_sorted = (sorted(self.rect_stds.items(), key=lambda x: x[1]))
    	self.std_filtered = [x[0] for x in std_sorted[:N_CANDIDATES]]
    ...
    def make_final_display(self):
    	...
    	for k in self.high_graded_rects:
    		canvas.create_text(txt_x, txt_y, anchor = 'w', font = None, text = "rect ={} mean elev={:.1f} std={:.2f} ptp={}".format(k, self.rect_means[k], self.rect_stds[k], self.rect_ptps[k]))
    		txt_y += 15
    	...
    	screen.mainloop()
def main():
	app = Search('670x335km')
	app.run_sect_stats()
	app.draw_rectangles()
	app.sort_stats()
	ptp_img = app.draw_filtered_rects(GRAY_IMG, app.ptp_filtered)
	std_img = app.draw_filtered_rects(GRAY_IMG, app.std_filtered)
